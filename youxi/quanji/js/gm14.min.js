function scs(e, t, n) {
	...
	switch(t) {
		case 1:
			for(i = 0; i <= r; i++) t = Math.floor(e.substr(i, 1)), spt_p(137 + t, rsdsx[r - i], n);
			2 < r && spt_p(147, 151, n + 34);
			break;
		case 2:
			for(i = 0; i <= r; i++) t = Math.floor(e.substr(i, 1)), spt(164 + t, rsdsx2[r - i], n);
			2 < r && spt(174, 198, 84);
			break;
		case 3:
			for(i = 0; i <= r; i++) t = Math.floor(e.substr(i, 1)), spt(164 + t, rsdsx2[r - i], n);
			...
			break;
		case 4:
			for(i = 0; i <= r; i++) t = Math.floor(e.substr(i, 1)), spt(175 + t, rsdsx3[r - i], n);
...
function Press(e) {
	...
	switch(gs) {
		case 0:
			175 < t && 365 < c ? goHome() : 60 > t && 350 < c ? setTimeout(function() {
				show_share()
		...
		case 5:
			...
			break;
		case 7:
			175 < t && 365 < c ? goHome() : 35 < t && 310 < c && t < 155 && c < 365 ? (scr = 0, vspnt[1] = 0, vspnt[2] = 0, vspnt[3] = 0, vspnt[4] = 0, ctx.clearRect(0, 0, 320, 416), st(), gldt2 = 4, ovblk = 0, setTimeout(function() {
				gs = 1
...
function lp() {
	...
	switch(gs) {
		...
		case 6:
			...
			0 < pfv && pfv--;
			r = 0;
			.1 < msi ? (1 == mss && (msx = Math.random() * msi * 2 - msi, msy = Math.random() * msi * 2 - msi, r = 1, msi = 1 == mst ? msi - 1 : msi - 2, ctx.clearRect(0, 0, 320, 416), ctx.save(), ctx.translate(msx | 0, msy | 0)), mss *= mst) : msi = msy = msx = 0;
			var s = bgi | 0;
			18 > s ? ctx.drawImage(img2, rgx[s], rgy[s], 320, 346, 0, 70, 320, 346) : ctx.drawImage(img8, rgx[s], rgy[s], 320, 346, 0, 70, 320, 346);
			.1 < esi ? (esx = Math.random() * esi * 2 - esi, esy = Math.random() * esi * 2 - esi, -20 > esy && (esy = -20), esi -= 1, sptp(elg, (ex + esx | 0) + 2, 416 + esy | 0)) : esi = esy = esx = 0;
			417 > edwy && spte(ei | 0, ex + esx | 0, esy + edwy | 0);
			e = etx - ex;
			ex += e / esp;
			bgb = 0 < edwn ? .8 * bgb : -32 > ex ? 1 : 60 < ex ? -1 : .8 * bgb;
			.1 > Math.abs(bgb) && (bgb = 0);
			bgi += bgb;
			29 < bgi && (bgi = 0);
			0 > bgi && (bgi = 29);
			s = ei | 0;
			if(51 != s || 0 == egd)
				if((25 == s || 40 == s) && 74 > mi) s != uei && (gdntm = 0, 48 < mi && 55 > mi ? (msi = 12, mss = 1, mst = -1) : (msi = 18, mst = mss = 1, 25 == s ? (mp = 4, pdmg(epja1, epja2)) : (mp = 5, pdmg(epsa1, epsa2)), ms = .25, mi = msf[mp], pfv = 0, 1 == pdwn && (msi = 24))), ei += es / 3;
				else switch(edwn) {
					case -1:
					case 0:
						ei += es;
					...
					case 1:
						58 != s && 63 != s && (ei += es);
						edwy += edwy / 4;
						416 < edwy && (edwy = 417, edwn = 9, edwct++, 2 < edwct ? (gtpnt = 10 * (pmpa * gtm * 60 / 40 + 10 | 0), 2 < vsi && gtpnt * vsi * vsi, 249990 < gtpnt && (gtpnt = 249990), scr += gtpnt, vspnt[vsi] = gtpnt, gs = 4, rstec = 0, a_submitScore(scr)) : (dwnc = 1, dwnac = 0, 80 > emp && (emp += 20, 121 < emp && (emp = 121))));
						break;
					case 2:
						edwy += -edwy / 3;
						ei += es;
					...
					if(100 > Math.abs(e)) {
						...
						switch(e) {
							...
							case 1:
								...
								break;
							case 2:
								etx = 120, esp = 10, epstc++, epc = epslg + Math.floor(20 * Math.random())
						}
						130 < Math.abs(ex - etx) && (esp = 12)
					}
					epdtm = 0
				} else Math.random() > epgop || 0 == eud ? epch() : (ep = 4, epe(), egd = es = 1, epc = Math.floor(Math.random() * epgdl) + epgdl, .87 < Math.random() && (epc = 50 + Math.floor(30 * Math.random())), eud = 0), epdtm++
			}
			if(0 < bp_fi) {
				var o = 0,
					u;
				for(u in bp_f)
					if(t = bp_f[u], 1 == t.l) {
						switch(t.tp) {
							case 0:
								gcp1();
								for(e = 0; 6 > e; e++) n = 30 * t.f + 5 * e, i = .8 * fani1[n + 4], 0 > i && (i = 0), ctx.globalAlpha = i, spt_pt(fanf1[e], fani1[n], fani1[n + 1], fani1[n + 2], fani1[n + 3]);
								ctx.globalAlpha = 1;
								t.f++;
							...
							case 1:
								gcp1();
								for(e = 0; 7 > e; e++) n = 35 * t.f + 5 * e, i = .8 * fani2[n + 4], 0 > i && (i = 0), ctx.globalAlpha = i, spt_pt(fanf2[e], fani2[n], fani2[n + 1], fani2[n + 2], fani2[n + 3]);
								ctx.globalAlpha = 1;
								t.f++;
							...
							case 2:
								gcp1();
								for(e = 0; 19 > e; e++) n = 95 * t.f + 5 * e, i = fani3[n + 4], 0 > i && (i = 0), ctx.globalAlpha = i, spt_pt(fanf3[e], fani3[n], fani3[n + 1], fani3[n + 2], fani3[n + 3]);
								ctx.globalAlpha = 1;
								t.f++;
								15 < t.f && (t.l = 0, delete bp_f[u]);
								gcp2();
								break;
							case 3:
								for(e = 0; 6 > e; e++) n = 30 * t.f + 5 * e, i = fani5[n + 4], 0 > i && (i = 0), ctx.globalAlpha = i, spt_pt_p(fanf5[e], fani5[n], fani5[n + 1], fani5[n + 2], fani5[n + 3]);
								ctx.globalAlpha = 1;
								t.f++;
								50 < t.f && (edwn = 0, rnds = 1, mt = tm = Math.floor(+(new Date) / 333), t.l = 0, delete bp_f[u]);
								break;
							case 4:
								for(e = 0; 6 > e; e++) n = 30 * t.f + 5 * e, i = fani6[n + 4], 0 > i && (i = 0), ctx.globalAlpha = i, spt_pt_p(fanf6[e], fani6[n], fani6[n + 1], fani6[n + 2], fani6[n + 3]);
								ctx.globalAlpha = 1;
								t.f++;
								28 < t.f && (3 == gs && (es = ms = 1, edwn = pdwn = 0), t.l = 0, delete bp_f[u]);
								break;
							case 5:
								for(e = 0; 5 > e; e++) n = 25 * t.f + 5 * e, i = fani8[n + 4], 0 > i && (i = 0), ctx.globalAlpha = i, spt_pt_p(fanf8[e], fani8[n], fani8[n + 1], fani8[n + 2], fani8[n + 3]);
								scs(gtpnt, 1, 211);
								ctx.globalAlpha = 1;
								t.f++;
			...
			if(51 != u || 1 != th)
				if(39 == u || 24 == u || 81 == u || 94 == u) mi += .3, 30 > Math.abs(ex - 14) && upps != u && (47 > s || 55 < s || 73 < u ? (esi = 12, (19 < s && 27 > s || 31 < s && 43 > s) && 74 > u ? (bp_f["c" + bp_fi] = {
					x: 0,
					y: 0,
					tp: 2,
					f: 0,
					...
					case 1:
						58 != u && 70 != u && (mi += ms);
						pdwy += pdwy / 6;
						160 < pdwy && (pdwy = 161, pdwn = 9, pdwct++, 2 < pdwct ? (gtpnt = 10 * ((121 - empa) * vsi * vsi * (edwct + 1) * 2 + 10 | 0), 249990 < gtpnt && (gtpnt = 249990), scr += gtpnt, vspnt[vsi] = gtpnt, gs = 5, rstec = 0, a_submitScore(scr)) : (dwnc = 1, dwnac = 0, 80 > pmp && (pmp += 20, 121 < pmp && (pmp = 121))));
						break;
					case 2:
						pdwy += -pdwy / 3;
						mi += ms;
			...
			if(mi > mef[mp]) {
				if(-1 == ut) mp = 0, mi = msf[mp], ms = 1, 1 == slwa && (ms = .3);
				else switch(ut) {
					case 3:
						grd();
						break;
			...
			if(0 < vtlpd) {
				...
				if(170 > gtm || 0 < edwct || 1 == vgd) vtlpd -= .04 + .05 * vgd, 0 > vtlpd && (vtlpd = 0);
				gdstm = 0
			}
			switch(gs) {
				case 3:
					...
					tm != mt && 1 < tm && 1 == rnds && (gtm--, 1 > gtm && (gtm = 0, 0 == onejdg && (gtpnt = 10 * ((121 - empa) * vsi * vsi * (edwct + 1) * 2 + 10 | 0), 249990 < gtpnt && (gtpnt = 249990), scr += gtpnt, vspnt[vsi] = gtpnt, gs = 5, rstec = 0, a_submitScore(scr), edwn = pdwn = 9, epc = 99999999, onejdg = 1)));
					mt = tm;
					break;
				case 4:
					for(e = 0; 5 > e; e++) n = 25 * rstec + 5 * e, i = fani8[n + 4], 0 > i && (i = 0), ctx.globalAlpha = i, spt_pt_p(fanf8[e], fani8[n], fani8[n + 1], fani8[n + 2], fani8[n + 3]);
					scs(gtpnt, 1, 211);
					spt_p(148, 50, 265);
					ctx.globalAlpha = 1;
					rstec++;
					...
					break;
				case 5:
					for(e = 0; 5 > e; e++) n = 25 * rstec + 5 * e, i = fani9[n + 4], 0 > i && (i = 0), ctx.globalAlpha = i, spt_pt_p(fanf9[e], fani9[n], fani9[n + 1], fani9[n + 2], fani9[n + 3]);
					scs(gtpnt, 1, 211);
					spt_p(148, 50, 265);
					ctx.globalAlpha = 1;
					rstec++;
					...
					break;
				case 6:
					ctx.globalAlpha = sovbl, ctx.fillStyle = "#000", ctx.fillRect(0, 0, 320, 416), ctx.globalAlpha = 1, sovbl -= .05, 0 > sovbl && (gs = 3, bp_f["c" + bp_fi] = {
						x: 0,
						y: 0,
						tp: 3,
						f: 0,
...
function init() {
	...
	for(e = 0; 10 > e; e++) msf[e] = n[2 * e], mef[e] = n[2 * e + 1], esf[e] = t[2 * e], eef[e] = t[2 * e + 1];
	e = t = n = 0;
	rgx = Array(30);
	rgy = Array(30);
	for(t = 0; 6 > t; t++)
		for(n = 0; 3 > n; n++) rgx[e] = 320 * n, rgy[e] = 346 * t, e++;
	for(t = 0; 4 > t; t++)
		for(n = 0; 3 > n; n++) rgx[e] = 320 * n, rgy[e] = 346 * t, e++;
	bp_f = {};
	bp_fi = 0;
